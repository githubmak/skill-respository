...
空间线稿、关键帧和视频必须先写入 `staging/`。工具不得根据失败报告自动移动人物、替换机位或改写画面；大模型完成创意修订后重新生成。线稿只有几何 PASS、真实画面 PASS 且 SHA-256 未变化时才能用 `promote_blocking_reference.py` 提升。Manifest 验证正式层完整库存，任何未登记媒体都使交付失效；`reports/` 与 `staging/` 仅保存证据，不属于正式交付。
...
完成盲答后才揭示导演合同与表演 IR，逐项比较 `情绪触发 | 关系意图/对外策略 | 角色专属身体或道具泄露 | 听者策略变化 | 句末残留 | 景别/机位/运镜服务`。任一项无法从直接提示词还原或与 IR 不一致，按 `shot` 返回编译前。以下情况同样返回 `REVISE + 最小 repair_scope`：答案只能复述台词、标签、焦段、颜色或运动参数；普通对白只有台词和通用微表情、听者反应不改变关系读取或句末状态全部重置；演法换名后可套给无关人物；连续用同一种微表情；以无因大动作冒充亮点；多人同时明显反应而无传播顺序；机位或运镜遮掉表演核心；签名镜核心晚于直接提示词前60%；关系/认知变化只在制作控制；构图删除后信息无损；运镜只改变大小或制造动感；光影只负责照明；情绪没有当前景别可见证据；普通镜可无损替换成通用中近景；相邻差异不足三项；多人关系、面向、可见面、轴线或边界无法物理闭合。其余才记 `PASS`。
