# 输出模板

## 必须使用的 Markdown 结构

只生成以下紧凑结构。它会把用户真正粘贴进即梦的内容，和复杂镜头才需要看的制作说明分开。

```markdown
# [标题] 即梦投喂分镜

## 使用说明
## 全局锁定
## 通用负面提示词｜直接复制
## 场景状态表
## 分镜投喂卡
## 附录｜原文保留检查（按需）
## 附录｜复杂镜头验证（按需）
```

普通交付不要输出两个附录，但内部仍然要完成对应检查。

## 全局锁定

把 `## 全局锁定` 写成简短、可复用的锁定块：

- `全局风格`：画幅比例、视觉风格、平台和质量词。
- `人物锁定`：人物稳定身份锚点，以及会反复出现的服装/身体状态。
- `场景锁定`：每个地点的 2-3 个固定锚点、光线方向、色温和背景控制。
- `状态锁定`：仅在剧情需要时，写入反复出现的道具或服装当前状态规则。
- `界面文字锁定`（仅必要时）：界面类型、侧边安全区、面部安全区、最多可读文字量，以及界面文字是后期叠字还是画内生成。

不要在每个镜头里重复这些全局块。单条视频生成时，把 `全局锁定 + 【画面描述｜直接复制】 + 可选的【必要约束】` 粘进正向提示词。

## 负面提示词

在全局锁定后立即创建 `## 通用负面提示词｜直接复制`，并只写一次以下基础负面词：

`五官漂移、换脸、脸型变形、发型错乱、服装变色、手指畸形、肢体穿模、多手多臂、非说话者口型乱动、口型错位、嘴部崩坏、背景重构、人物瞬移、站位互换、道具漂浮穿手、画面跳帧、过度磨皮、模糊失焦、夸张翻白眼、人物僵硬、全身静止、无眨眼、空洞呆滞眼神、面部无任何变化、肢体不动、木偶式静止、死板、定格、面部僵硬`

只有某个单独镜头确实有额外风险时，才在该镜头旁边补充镜头专属风险。不要把这整段负面词复制到正向提示词里。

## 场景状态表

每个地点或状态变化使用一条精简记录：

`场景/状态ID | 固定锚点和光线 | 人物物理关系和画面区域 | 活动道具/服装持有人和位置 | 允许变化的事实`

只列会跨多个镜头延续的事实。不要为同一信息重复创建人物位置表、道具表和空间表。

多场景故事在本节下添加简短的 `### 本集关键状态线`。只追踪跨场景变化，例如 `陆序穿着外套 -> 沈星雨披着外套 -> 沈星洲穿着外套`；不要重复 `【状态继承】` 已覆盖的逐镜细节。

## 镜头组和子镜头卡

`#### S1-01｜镜头组总时长：7.5s` 是便于阅读的剧情节拍组，不是额外的即梦提示词。它代表一个具体的观众认知变化。可见人物只在组级写一次，但组级 `【出现人物】` 只是本组曾经可见的人物索引，不要求每个子镜头拍全，也不要随单条子镜头一起投喂即梦。把组内所有子镜头时长相加，并写在组号旁边。连续的 `【镜号】1/2/3` 是分别生成的即梦子镜头；每条都有自己的时长和一个主要任务。不要添加 `分镜1` 这类标题。

```text
**场景一｜商业街**

#### S1-01｜镜头组总时长：4.5s

【出现人物】
沈星雨
宋南枝

【镜号】
1，2s，普通。

【画面描述｜直接复制】
[实际可粘贴进即梦的正向提示词，通常 220-380 个中文字符，绝不超过 500 个中文字符。要自然包含：景别 + 机位/角度 + 静止或一条路径 + 人物/道具关系 + 一个动作或台词落点。若本子镜头有可见人物说台词，原台词必须默认写在这里。]

【表演与声音】
[只写本子镜头新增的表演阶段和声音。]

【状态继承】
[下一子镜头或下一组必须继承的尾帧可见事实。]

【镜号】
2，2.5s，普通。
...
```

每个子镜头都按以下顺序包含四个默认字段：

```text
【镜号】
[本组内子镜号]，[时长]s，[普通 / 复杂]。

【画面描述｜直接复制】
[实际可粘贴进即梦的正向提示词，通常 220-380 个中文字符，绝不超过 500 个中文字符。可见人物说出口的原台词默认必须写在这里；除非用户明确要求不写入画面描述，或该句是 OS/OV/系统音/内心独白/画外声音/后期配音。]

【表演与声音】
[只写本子镜头正在发生的对白、OS、OV、系统音、环境声或必要反应：时间、口型规则、语速、停顿/尾音和动机。可见对白可以在这里重复用于口型和声音控制，但不能只出现在这里；必须已经原样进入 `【画面描述｜直接复制】`。OS、OV、系统音、内心独白、旁白只要有具体文本，必须写成 `OS：“原文。”`、`OV：“原文。”`、`系统音：“原文。”` 这种冒号加引号格式。只写本子镜头新增的表演阶段，不重复完整情绪链。两层或更多声音重叠时，写清声音优先级：可见对白 > 关键音效 > OS/系统音 > 环境声。没有台词时写“无台词”。]

【状态继承】
[下一镜开头需要继承的当前可见事实：位置/朝向、道具持有人、服装、情绪余波和下一镜锚点。]
```

写子镜头前，内部先建立这一行声明，再把它翻译成自然中文写进 `【画面描述｜直接复制】`：

`本镜可见主体 | 主表演者 | 必要听者/前景肩线 | 必要道具 | 固定锚点 | 身体朝向锚点 | 头/眼转向（按需） | 相对站位或接触物 | 景别容量是否匹配 | 机位侧/角度 | 静止或单一路径 | 本镜唯一落点`

不要输出这条内部声明。手部/物件插入镜头只写画面中真正出现的手和道具；关系镜头要写出每个可见人物；近景/特写只写主脸或主道具和必要边缘肩线，不要为了组级人物表让无关角色入画。不要使用兜底主体、条件朝向或泛泛机位词。

## 可选字段

只有可选字段能防止真实生成失败时才添加。不要添加空占位字段。

```text
【空间与道具锁定】
[用于三人及以上可见人物、桌/门/车等物理关系，或任何道具转移。写清手能否触达、移动路径和松手状态。]

【镜头执行】
[用于非静止运镜、焦点变化或视觉微动作。写清起始停顿、路径、焦点目标和落点画面。]

【口型分窗】
[用于两个或更多可见说话者、超过 5 秒的对白，或口型交接有风险的镜头。开头必须写 `优先级：口型 > 听者反应 > 运镜`。每个时间窗只写一个可见说话者，并写清何时闭嘴。如果同镜还有运镜，要说明听者除呼吸/视线外保持不动；否则取消运镜。]

【镜内状态转换】
[仅在本卡对白或反应前，可见改变身体姿态、手/道具接触或归属、服装、门窗、灯光、UI、人物距离、说话状态或事件结果时使用。写成 `起始：...；转换：...；终态直投句：...；台词/反应：...；尾帧直投句：...`。两个直投句必须原句出现在 `【画面描述｜直接复制】` 中。]

【剪辑衔接】
[用于场景/时间变化、闪回、连续道具转移/动作衔接，或独立物件/空景/结果切入。独立切入镜头必须写 `独立生成；后期接入...声音`，并写清前后状态锚点。]

【必要约束】
[最多三条本镜头专属正向约束或风险提示。若组级人物较多但本子镜头只需要部分人物，为防止 AI 补人导致穿帮，可写 `本镜只出现A和B，C不入画`；不要把这类限制泛化到不相关镜头。]

【校验记录】
[只用于复杂镜头；不要粘贴进即梦。]
```

## 直接复制提示词规则

- 直接提示词必须能配合全局锁定独立成立，并重写上一镜改变后的当前可见状态。
- 权重分两层：第一层先过不穿帮硬门槛，人物、道具、面向、距离、口型和单镜任务必须稳定；第二层再按 `剧情节拍与观众认知 > 情绪表演×景别运镜匹配度 > 声音氛围和装饰细节` 取舍。情绪和镜头是一组匹配关系：情绪决定为什么这样拍，景别和运镜负责让情绪被看见。不要因为保稳定而连续输出平淡固定镜头。
- 信息写作顺序：当前固定锚点 -> 每个可见人物身体面向哪个人物或固定物 -> 头/眼方向与身体不同时再写头/眼转向 -> 相对位置/道具关系 -> 景别和机位/角度 -> 静止或一条路径 -> 一个主要动作和可见情绪 -> 台词口型或后期声音处理方式 -> 尾帧状态。`画面左/右` 只能补充关系，不能替代身体方向、距离、隔物或机位侧。
- 可见人物说出口的台词默认必须写进直接提示词，保留原文引号、标点和省略号。写法示例：`沈星雨侧身停稳后开口说：“原文台词。”`。`【表演与声音】`、`【口型分窗】` 可以重复台词做口型和节奏说明，但不能作为台词唯一位置。只有用户明确说不要放进画面描述，或声音属于 OS/OV/系统音/内心独白/画外声音/后期配音时，才允许不写入直接提示词。
- OS、OV、系统音、内心独白、旁白类声音若包含原文，必须使用 `标签：“原文。”` 格式，不能裸写文本，也不能只写 `OS以低音量响起：“原文。”`；应写 `OS：“原文。” 低音量，语速偏快，画面中人物闭口。`
- 普通对白/剧情镜头必须达到最低画面密度：固定锚点、人物物理关系、前后层级或遮挡、身体朝向、道具状态、一个可见情绪泄露动作、镜头静止理由或单一路径、尾帧余波。手部特写、纯物件插入、空景可以更短，但也要写清道具/固定物起点和尾帧。不要输出只包含 `地点 + 人物站位 + 台词` 的瘦提示词。
- 情绪反应必须被拍出来。重要反应至少写出两类可见证据：面部控制、身体姿态、手/道具动作、距离变化、遮挡变化或镜头落点。只写 `闭口、皱眉、看着对方` 不够；如果反应本身是剧情落点，必须使用独立反应镜头、肩后反打、近景或道具插入承载。
- 表情必须按景别写到可读：近景/特写写眼睑、眉尾、嘴角、下颌、喉咙、呼吸；中近景写脸部加肩背/手；中景/全景不要依赖细微表情，要用身体距离、手部大动作、道具、遮挡和机位变化表现情绪。不要把 `眼神复杂、微微皱眉、表情平淡` 当成主要表演。
- 景别有承载上限：特写/近景最多承载一个主脸或一个手部/道具，以及一个边缘肩线；中近景承载一个说话者加一个听者肩线/低幅反应；中景承载二到三人关系和大动作；全景/中远景只建立空间、动线和群体关系。若本镜需要三人以上关系，先用中景关系镜头；若要表情爆点，再切近景。
- 柜台/窗口/签字台场景必须使用可见内外侧词，但不要固定套用示例左右。先确认本镜真实固定物和人物：柜台/窗口/签字台是否存在、工作人员是否入画、办事者在柜台外侧还是已转身离开、同伴是否同在柜台外侧且可触达、道具在手里还是台面上。不要写 `来客位`、`办事位`、`工作人员位`、`服务位`。可用词包括：`柜台内侧`、`柜台外侧`、`签字台外沿`、`窗口内侧`、`画面右前方/左后方`。示例只展示写法，不继承左右和人物数量：`民警在柜台内侧画面左后方，沈星雨站在柜台外侧的签字台前画面右前方，二人之间隔着签字台和柜台。`
- 景别变化、反打、肩后、行走节拍、离场节拍、柜台/门/道具动作之后，新对白/反应卡必须重新写完整身体朝向。普通面对面对话要先写 `A身体面向B，B身体面向A`，再写机位。如果上一镜尾帧人物面向门、道路、柜台、手机或第三人，下一镜必须重写已变成面对面的身体朝向，或先展示转身后再开始对白。
- 如果上一镜 `【状态继承】` 或尾帧写了背向/背对/侧身/身体面向固定物，下一镜不能直接改成面对某人。直接提示词必须在开头写出可见动作链：`A身体从...转向B，肩线转正，双脚停稳；A身体面向B后再开口/接物/反应。` 如果这条镜头还要长台词、递物或复杂运镜，先拆一个定位子镜头。
- 成对正反打必须保留成对写法：`机位在A肩后拍B，A前景肩线弱虚化` 的反打是 `机位在B肩后拍A，B前景肩线弱虚化`，同时重复相同距离/隔物、道具状态和背景锚点。不要写 `B右前方` 或 `侧前方` 这类松散反打，它会破坏 AI 生成轴线。
- 道具转移提示词必须防止物品闪现和不可触达：写清道具从哪里开始、谁的哪只手取出或呈递、接收者如何处于可触达位置、何时接触、何时松手、尾帧道具停在哪里。如果接收者开头在身后/侧后/背向/不可触达，先写一个定位子镜头让接收者转身或让递交者走到其面前；不要把重新站位、取道具、递交和对白挤在同一子镜头。
- 跨镜物品状态必须一致。上一镜 `【状态继承】` 写了某物在A右手/包内/口袋/桌面，下一镜开头必须重写同一事实；若物品要变到B手里、另一只手或另一表面，直接提示词开头必须拍出完整转移链。不要让 `银行卡仍在A右手` 下一镜突然变成 `银行卡在B手中`，除非中间有接触与松手。
- 每个子镜头都要写景别和镜头状态。静止镜头要写机位侧/角度和画面承载任务；运动镜头要写一条路径和落点目标。不要只写 `中近景固定`，也不要让镜头信息隐含。
- 不要把标题、检查清单、禁止事项或通用表演理论写进直接提示词。
- 如果提示词需要超过一个主要生成任务，就拆分。子镜头必须重写当前可见事实，不要写 `继承` 或 `延续上一镜`。
- 同卡状态变化按时间顺序写：开头状态、一个可见变化、稳定结果，然后才是可见对白/反应和尾帧状态。把 `【镜内状态转换】` 里的 `终态直投句` 和 `尾帧直投句` 原句复制进直接提示词。不要把手机同时写成“贴着耳朵”和“已经放下”，不要把门同时写成“开着”和“关上”，不要在没有接触/松手链的情况下把道具写成两个人同时持有。
- UI、手机文字、付款提示、系统叠字要预留侧边安全区，避开脸、嘴和活动手。除非用户明确要求画内生成清晰文字，否则密集文字都按后期叠字处理。
- 独立物件/空景/结果切入必须添加 `【剪辑衔接】`。在 `【必要约束】` 中只写真正需要的约束，例如 `本镜无人入画；不生成口型；保持上一镜同场景光线和道具终态`。不要要求模型在同一条生成视频里硬切“脸 -> 空景 -> 脸”。

## 闪回衔接

回忆、梦境、蒙太奇或时间/地点变化使用独立片段。现实触发镜头结尾保留 0.5-0.8 秒稳定身体停顿；插入片段开头保留 0.5-1.0 秒稳定场景锚点。直接提示词只写可见状态；淡入淡出或声音桥写在 `【剪辑衔接】`。

## 可选附录

`附录｜原文保留检查` 是精简源文节拍核对表。`附录｜复杂镜头验证` 只列复杂镜头编号、首轮生成风险和需要优先观察的结果。两个附录都不属于普通即梦投喂流程。
