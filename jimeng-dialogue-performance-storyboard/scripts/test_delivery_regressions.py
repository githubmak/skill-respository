...
- **起幅与连续性**：客厅白天，林岚与周启两人隔桌站立；林岚位于桌面右侧、身体朝向周启、视线侧避，重心稳定并维持冷静身体基线，右手悬在文件上方；周启位于左侧并正对林岚；文件平整留在两人之间，当前控制者为林岚、持物手为右手但尚未接触；门口位于林岚身后且路径未被占用，保持桌面关系轴；焦点落在林岚右手与文件，人物处于右侧窗光与左墙反射交界，室内底噪持续。
- **摄影与构图**：{photography}。
- **光影、环境色彩与材质**：低饱和冷灰蓝位于背景和窗外空气，柔和暖白落在人物与桌面反射区，不覆盖人物服装、暗木桌和文件固有色；右侧窗光横过桌面，左墙提供低强度柔和反射，林岚面部按中全景级保护保持自然肤色、可读中间调和暗侧纹理，背景亮度略低；人物移动只连续改变受光面，关键手部受光和白平衡保持稳定；文件纸面呈克制漫反射并在滑动时产生真实摩擦，暗木桌保持低亮度细纹反射；林岚退向门口时侧脸亮面逐渐收窄，停步后稳定。
- **画面与表演时间线**：{timeline}。
...
class DeliveryValidatorTests(unittest.TestCase):
    ...
    def test_missing_core_field_is_blocked(self) -> None:
        text = re_sub_once(r"(?m)^- \*\*光影、环境色彩与材质\*\*：[^\n]+\n", "", direct_document())
