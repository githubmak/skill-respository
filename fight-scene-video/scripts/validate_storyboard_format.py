#!/usr/bin/env python3
"""Validate the fixed storyboard contract and obvious Seedance execution risks."""
...
def validate(text: str) -> list[str]:
    ...
    for shot_index, match in enumerate(SHOT.finditer(normalized), start=1):
        ...
        sequence_limit = 5 if duration <= 3.0 else 8 if duration <= 6.0 else 11
        if duration <= 8.0 and sequence_count >= sequence_limit:
            errors.append(
                f"镜号 {shot_index:02d} 为{duration:g}秒，却含{sequence_count}个顺序标记；"
                "疑似超过一个核心动作的执行预算，请保留画面事实并拆镜。"
            )
...
def self_test() -> int:
    ...
    invalid_overload = valid.replace(
        "深夜都市背街，巷心伫立宋棠一人。",
    ...
    cases = (
        ...
        bool(validate(invalid_overload)),
