...
导出必须通过 `direct_prompt_compiler.py` 按“视觉前缀→空间→连续性→表演→光影→视频质感→电影质感”组装：只做跨段精确去重，不按类别、关键词或优先级删除创作句，不得从句中截断。空间、连续性、表演、光影和原生音频台词属于模型创作事实，必须作为受保护事实保留；结果超过700字、导演卡超过500字或必须改变语义时，Export 返回 `CREATIVE_REWRITE_REQUIRED` 并退回 Master Production 重写。

完整直投正文之外必须由模型填写 `director_card` 并控制在500字以内；Export 只验证和排版，不从完整提示词派生语义摘要。默认另导出 `.concise.md` 简洁交付视图和 `.engineering.md` 工程审查视图；前者只含模型导演卡与负面词，后者保留句级 source ID/Scene Lock/合同溯源和质量审查，不得投喂模型。
...
- `reroll_control.mitigation_steps` 中只有以 `可见结果：`、`画面结果：` 或 `降级后画面：` 明确标注的具体画面事实进入 production-control grounding；锁定步骤、人工检查、失败重试、删除次要响应和后期处理仍属于制作控制，不要求逐字写入直投正文。
