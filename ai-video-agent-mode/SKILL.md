...
description: >
  合同驱动的 AI 视频提示词生产流程：把剧本、分镜或场景
  转换为可直接生产的 AI 视频提示词包，并控制抽卡风险、人物表演张力、动作预算、
  当前镜头剧情关键帧、角色优先级、即梦 T2V 生成规则、跨镜连续、语义审查和 Markdown/XLSX 导出。
  适用于 agent模式、多agent、剧本转AI视频提示词、专业分镜提示词、降低抽卡率、
  表演增强、当前镜头剧情关键帧、Seedance/即梦视频提示词。
---
...
当前契约固定为即梦 T2V。允许交付即梦主镜头提示词、负面提示词、台词/OS/OV与配音控制、连续性/摄影/表演/抽卡风险数据，以及当前镜头单张剧情关键帧生图提示词；不提供 I2V、R2V、参考素材槽位、首尾帧、动作素材或九宫格剧情包能力。
...
2. Phase 0 必须使用 `scripts/resolve_run_mode.py` 和 `scripts/configuration_wizard.py` 逐轮确认基础配置，绝不一次性列出整张配置问卷：第一轮只问 `export_base`；随后每轮只问 1–2 项，固定顺序为“画幅+视觉风格 → 最大时长+目标平台 → 原生音频 → Markdown 交付路径”。交付路径写入 `delivery.markdown_path`，必须位于 `export_base` 下；通过最终验证后自动导出，绝不重复确认。生成方式固定写入 `t2v`，目标平台固定为即梦。若用户或平台适配器明确提供提示词字符硬上限，写入 `prompt_limits.hard_max_chars`；否则保持 `null`，不得猜测。
...
19. 把平台最大时长视为单片容量，但不得为了填满容量合并第二个剧情节拍：一个主镜头只服务一个 `narrative_beat_id`。主镜头不是一个画面，而是一个单一戏剧问题：谁想要什么、被什么阻挡、观众此刻好奇什么、落幅留下什么未完成状态。`shot_group` 的 1–3 个子镜不是独立镜头，只能是该节拍的起势、承接、转折或落幅，`duration_design.dramatic_beats[]` 只记录这个单一剧情节拍内部的可见因果片段；出现新的事件目标、主动作链、反应归属、情绪结论或第二次注意力交接时，必须拆成下一主镜。Phase 1 的动作时长必须使用与 `validate_durations.py` 相同的下限估算；对白镜只按已锁定逐字台词与必要反应空白计时，不能把台词中的偶然动词重复算作物理动作。禁止为了利用率添加氛围、静默、凝视或重复余韵。单一微表情、一次视线变化、静态压场或群体凝视默认不超过 `max_static_shot_duration`（默认 6 秒）；超时子镜必须在唯一 `duration_design` 对象中提供 `duration_strategy=pack_toward_limit`、`justified_content_duration`、`utilization_ratio`、`duration_rationale` 与有序 `dramatic_beats[]`。
...
22b. 子 Agent 必须按字段接力调度：Emotion Agent 先产出 `emotion_driver` 与 `performance_contract` 的核心表演重音；Scene Agent 产出不可变 `scene_lock`；Camera Agent 只能消费 `emotion_driver`、`coverage_role`、`scene_lock` 与 `continuity_contract` 设计 `camera_beat_map`，不得新增情绪、剧情或道具事实；Composer Agent 只能把 `dramatic_design`、`performance_contract`、`continuity_contract`、`scene_lock`、`camera_beat_map` 合成为五段式 `full_prompt`，不得重新发明运镜。任一后置 Agent 发现上游字段不足，只能写入缺口与风险，不能用自由创作补齐。
23. Director 必须为每个主镜选择 `editorial_mode`：`continuous_take` 为一条连续摄影轨迹；`shot_group` 为同一即梦生成任务内 1–3 个可见子镜。无论哪种模式，该主镜只拥有一个剧情节拍；子镜组只能服务该节拍的内部表演重音，不得把“侍卫拖拽”和“皇后冷漠审视”这类可独立成戏的动作/反应节拍塞进同一主镜。每个子镜必须写时间窗、主体、景别、屏幕左右、人物朝向、前景肩膀/场景锚点、动作表演和落幅承接；切换只能由本节拍的表演重音触发。同一 T2V 任务只允许一次单向人物注意力交接（如 A→B）；`A→B→A`、两次人物回切或需要精确剪点的反打，必须拆成下一条 T2V 任务，并以落幅/起幅、声音边界和动作匹配点交给后期。
24. Phase 1 必须生成 `source_ledger.json` 与 `dramatic_beat_ledger.json`。每个原文事实、动作、台词和 OS/OV 可回溯到源位置；每个主镜锁定一个 `narrative_beat_id`，每个 `dramatic_beat_id` 只有一个 `owner_subshot_id` 并只能作为该主镜剧情节拍内部的可见片段。相邻镜头只能通过 `reserved_by` 预留或通过连续性字段承接，不得重复演绎或跨主镜抢同一节拍。
...
27. 50 个主镜头的核心链路目标为 P95 ≤55 分钟。`performance_budget.py` 必须基于真实 pipeline state 输出阶段耗时、dispatch 数、重试数与是否达标；没有三套 50 镜基准（对白、动作、混合）及注入 10% 失败的重试结果，不得声称已达到该目标。
...
| Phase 10 export | Markdown + XLSX | Markdown 为每主镜导出即梦操作卡、正文、负面词、当前镜头剧情关键帧、子镜节拍与台词表演 |
...
| Phase | 名称 | 执行方式 | 门禁 |
|---|---|---|---|
| 0 | 用户确认 | 逐项生成并确认 `project_config.json` | 每轮仅 1–2 项；确认元数据完整，画幅、风格、最大时长、平台、音频、T2V 模式与导出位置齐全；`run_dir` 已建立且位于 `export_base` 下 |
...
- `continuity_contract`：锁定本镜起点、落点、人物位置、视线方向、道具状态、光源关系和下一镜必须继承的残留，防止单镜好看但跨镜断裂。相邻主镜必须至少使用一种承接桥：动作承接、视线承接、道具状态承接、声音桥、空间轴线承接或情绪残留承接；下一镜起幅不得只写人物名称，必须继承上一镜落幅的姿态、视线、道具接触、重心、屏幕左右、焦点关系与同源光。
- `reroll_control`：按身份、服装、左右站位、动作、场景、运镜和口型评估 T2V 抽卡风险；人物镜不得标低风险，rising/peak 人物镜必须标记人工首轮验证。
- `dramatic_design`：记录 `shot_function`、`narrative_weight`、`information_gain`、`reaction_ownership`、唯一 `narrative_beat_id` 与本镜内部的 `dramatic_beat_ids`，确保重要人物出场、揭示和反应镜有明确戏剧任务且不重复原文节拍。需要短剧钩子时，`curiosity_hook` 只能由“异常可见结果 → 人物即时反应 → 暂不揭示的原因 → 马上到来的代价”构成；每个主镜最多保留一个开放问题，且不得凭空新增设定。
...
每个主镜的语义调度按以下接力执行，任何阶段不得跳级：

1. Orchestrator 锁定 `narrative_beat_id`、`dramatic_design.coverage_role`、`editorial_mode`、动作预算与子镜数量上限；只说明本镜戏剧问题，不写完整提示词。
2. Emotion Agent 基于源文产出 `emotion_driver`：`trigger`、`start_state`、`visible_leak`、`face_or_eyeline`、`voice_or_breath`、`end_residue`、`tension_intent`、`empathy_anchor`。它只定义可见表演重音，不选择炫技运镜。
3. Scene Agent 产出 `scene_lock`：人物左右、前中后景、道具归属、场景锚点、光源类型/方向/色温和不可变服装事实。后续阶段只消费，不覆盖。
4. Continuity pass 产出 `continuity_contract`：起幅、落幅、承接桥、人物/道具/视线/重心/光源继承和下一镜残留。
5. Camera Agent 消费 `emotion_driver`、`coverage_role`、`scene_lock` 与 `continuity_contract`，输出 `camera_beat_map`、`camera_pressure`、`shot_size_reason`、`movement_lock`、`cut_or_focus_trigger` 与 `risk_notes`。每一条镜头变化必须绑定 `visible_leak`、`face_or_eyeline`、`voice_or_breath`、道具状态变化或台词落点之一。
6. Composer Agent 消费上述全部合同，按五段式写 `full_prompt`。它只能把 `camera_beat_map` 翻译进 `子镜头组`，不得新增推拉摇移、反打、空镜、道具特写或注意力交接。
7. Editor Pass 2 审查字段接力是否断裂：Camera 是否越权新增表演或剧情，Composer 是否重设运镜，子镜是否丢失上一落幅，运镜是否没有情绪触发点。

派发 packet 必须携带上游字段摘要，至少包含：

```json
{
  "shot_id": "S1-01",
  "narrative_beat_id": "NB-01",
...
Camera Agent 的派发指令必须包含：只根据 `emotion_driver`、`coverage_role`、`scene_lock` 与 `continuity_contract` 设计 `camera_beat_map`；固定机位优先；只有表演重音、道具状态、视线或台词落点触发时才允许极慢推近、拉焦、单向重构图、硬切或跟随之一；不得写 `full_prompt`、不得改台词、不得新增空间事实。
...
- 运镜 Agent 不做独立导演创作，只把 `emotion_driver` 中的可见表演重音翻译为 `camera_beat_map`。若 `emotion_driver.visible_leak`、`face_or_eyeline`、`voice_or_breath` 与 `end_residue` 都不足以支撑镜头变化，必须选择固定机位或提示上游补充表演重音，不得为了丰富画面添加推拉摇移、反打、空镜或道具特写。
- `camera_beat_map[]` 每个子镜必须包含 `time_range`、`focus_owner`、`focus_subject`、`framing`、`trigger`、`camera_response`、`camera_position`、`camera_movement`、`transition_type`、`screen_lock`、`axis_relation`、`axis_carryover`、`carryover` 与 `end_frame`。其中 `trigger` 必须逐项指向 `emotion_driver`、台词落点、视线变化或已确认道具状态变化；`camera_response` 必须说明观众为何需要靠近、远离、旁观、跟随或转移注意力。
- 运镜响应优先级固定为：表演因果清晰可见最高；空间轴线与道具归属稳定次之；景别变化和运动美感最低。若运动会削弱表演可读性或空间稳定，固定机位优先。
...
- 景别与运镜必须服务戏剧功能，不按配额轮换：远景/全景用于建立空间、危险距离和行动路线；中景用于关系阻挡、身体重心和道具归属；中近景用于压迫、犹豫、倾听和台词承接；特写只用于关键情绪泄露、道具状态变化或伏笔揭示。推近表示压力收紧，拉远表示孤立、失败余波或信息落空，横移/跟随表示权力位置或行动路径改变，拉焦只表示一次注意力交接；每次镜头变化都必须由动作、视线、道具或台词落点触发。
- 对手戏可使用“心理距离景别反差”，但只在源文形成明确不对等时启用：焦虑、被逼问、急迫或失控的一方可用近景/特写、背景压缩、轻微推近来表现世界收窄；松弛、掌控、旁观或不接招的一方优先用中景/宽景、固定镜头、桌面/门框/屏风等空间留白保持呼吸感。它是剧情策略，不是景别公式；不得为了反差强行让每次对话都一近一宽。
- 插入镜头定义为：在主叙事镜头或双人互动中短暂切入局部特写、细节、空镜、反应或时空意象，再回到主线。AI 漫剧只能把它当作当前 `narrative_beat_id` 的辅助证据，用于补充关键信息、放大单一情绪泄露、调节长对话节奏、标记道具/伏笔、缓冲转场或保留环境残压；不得为了“丰富画面”插入无关空镜。
- 插入镜头稳定性优先级：已确认道具特写/环境残留最高；手部、衣角、杯沿、文件等局部中高；半脸/眼神特写中等且只给关键破防；多人反应、连续快切、回忆/幻想插入为高风险。高风险插入若承载新事件、新时空、新反应结论或第二次注意力交接，必须拆为下一主镜，回忆/幻想优先走 `temporal_transition_contract` 或独立主镜。
- 插入镜必须带来新的可见信息、道具状态、关系压力或情绪残留，并继承同一轴线、光源、左右站位、场景锚点和插入前后的动作状态。主镜内保留插入时，通常只允许 1 次动机切入；6–10 秒可最多 2 次但不能回切形成 `A→B→A`；人物脸部大特写不得连续使用。
...
3. `主镜头连续规则：` 一个剧情节拍、一个戏剧目标、同一光源/空间/人物关系，以及本主镜整体的连续性约束。
...
Composer 按以下注意力顺序写每个子镜：`承接上一落幅 → 消费 camera_beat_map 的当前景别/机位/运镜 → 当前景别的实焦主体与屏幕位置 → 本段唯一触发/动作 → 可见表情与身体承接 → 台词或OS的口型边界/语气 → 信息增量 → 场景光声与当前必要稳定约束 → 落幅残留 → 禁止变化项`。前半句只出现本段最重要的 1 个主体、1 个动作和 1 个画面证据；已在本主镜锁定的画幅、画风、光源、服装、位置或禁令不得逐段重复。Composer 不得新增 `camera_beat_map` 未声明的推近、拉焦、重构图、硬切、反打、空镜或道具特写；如发现镜头响应不足，只能写入 `qa_metadata.risk_notes` 或退回 Camera，不得自行补镜头。

### 4.1.0 最终提示词整合层

Composer 按 `references/format_constraints.md` §B0/§B2 的“主镜头状态机 → 子镜头状态机”装配最终提示词。用户提供 `主镜头：{ 子镜1：子镜2： }` 时，保留层级：主镜头只写全局不可变状态和唯一戏剧问题，子镜只写随时间变化的动作、表演、镜头响应和落幅继承。

`主镜头连续规则` 固定按以下顺序压缩为一段：

```text
单一剧情问题 → 情绪/冲突触发 → 空间状态总锁 → 道具生命周期总锁 → 运镜响应原则 → 本镜禁止变化项
```

- 单一剧情问题：谁想要什么、被什么阻挡、观众此刻好奇什么；不能写第二个剧情目标。
- 情绪/冲突触发：只写 `emotion_driver.trigger` 中能被画面看见的触发点，不写“张力强、悬念足”等结果词。
- 空间状态总锁：用画面左/中/右、前景/中景/后景、身体朝向、视线方向、真实接触点表达；少用“旁边、对面、身后、靠近、远处”这类缺少屏幕坐标的词。
- 道具生命周期总锁：起始位置 → 朝向 → 归属 → 接触状态 → 可移动条件 → 结束位置 → 下一镜继承。关键道具不允许无动作依据地从桌面闪现到手上，或从手中复位到桌面。
- 运镜响应原则：只说明 camera_beat_map 已授权的一种响应，如固定、极慢推近、单向重构图、拉焦、硬切、细节切入或跟随；没有触发点时保持固定。
- 本镜禁止变化项：只列当前最容易穿帮的 2–4 项，例如不镜像翻转、不越过人物连线、道具不离开桌面、非说话人口型闭合。

每个 `子镜头组` 的时间窗固定按以下顺序写：

```text
承接上一落幅 → 人物位置/朝向/视线 → 道具位置/朝向/接触状态 → 唯一触发/动作 → 情绪可见证据 → 镜头响应 → 结束状态/下一镜继承 → 禁止变化项
```

硬门槛只在这里记口诀，详细写法以 `references/format_constraints.md` §B0 为准：每个子镜只有一个实焦主体、一个动作/状态变化、一个表演证据和一个落幅；多人站位用屏幕坐标与景深层级；关键道具写生命周期；物品转移、人物转向、视线/重心/站位变化必须写 `state_transitions[].intermediate_state` 并落到 `子镜头组`；运镜只能在动作或情绪证据之后响应触发。

当骨架提供 `temporal_transition_contract` 候选时，Composer 先作合同决定再写正文：`memory_flashback` 仅在源文有明确记忆与可拍过去事实时可启用；`story_event_transition` 仅在当前源文事件确实造成场景、意识、时空或人物状态切换时可启用。二者都可在 `decision_reason` 说明为何正常切换更忠实。启用时，先由当前事件提炼 `effect_source_basis`，再在子镜头组写一次合同中唯一的效果及其时间窗、前后状态、声音桥和闭口边界；`prompt_anchor`、`audio_bridge` 必须逐字出现。不得套用固定特效、叠加多种效果、凭空增加过去事件，且必须将人物、服装、空间或时代变化写成合同中的明确后态。若生成不稳，按 `fallback=split_with_matched_cut` 拆成相邻 T2V 片段承接，不以堆叠特效补救。
...
- 对手戏先判断是否存在心理距离差：一方是否被压缩、急迫、求回应或失控，另一方是否松弛、掌控、回避或拒绝进入对方节奏。若存在，`performance_contract.camera_pressure` 与 `scene_pressure` 必须说明景别、焦点、空间留白或遮挡如何服务这个差异；若不存在，不为了“丰富画面”硬套特写/宽景反差。
...
- 插入镜头计入 `shot_group` 和镜头响应预算：3–6 秒最多 1 次；6–10 秒最多 2 次；10–15 秒仍最多 3 个子镜总量。插入镜如果只是静态氛围、无新信息或只重复情绪，应删除而不是占用时长。
...
- 打斗子镜按“起始距离/站位 → 攻击意图 → 接触点或闪避判断 → 受力方向/重心变化 → 结果状态 → 下一镜继承锁”书写。每个接触节拍只允许一个清晰结果：击中、格挡、闪避、失衡、反制准备或主动收住；禁止同一子镜同时写多段连招、换轴、回切、夸张慢动作和复杂环绕运镜。
...
- 短剧张力优先来自信息差、倒计时、可见代价、状态反转和未完成动作。常用可见压力物包括即将关上的门、快熄灭的灯、将落未落的杯子、屏幕亮起的消息、被握紧的钥匙、伤口加重、逼近的脚步声、被打断的台词；每个主镜只能选择一个压力物或一个压力机制，不能堆叠悬疑符号。
- 张力强度服从原剧本，不以“所有镜头都拉满”为目标。低张力镜保持自然流动，高张力镜通过更清晰的因果、反应延迟和终态残留增压，不靠增加动作数量。
- `rising/peak` 镜头必须建立 `pressure_release_design`：压力来源 → 唯一压力物/压力机制 → 1–2 个可见升级点 → 释放触发 → 释放结果 → 拆镜阈值。制造压迫优先使用已确认道具、门、脚步声、倒计时、屏幕消息、封条裂口、未完成动作或人物距离变化；释放压迫必须是动作完成、被打断、注意力转移、代价揭示、延迟悬置或拆到下一镜，不能只写“压迫感增强/气氛释放”。
- 压力物不是装饰符号。一个主镜只选一个压力物或压力机制，并写清它如何改变观众认知：例如普通药瓶只有位置意义，贴有裂开的红色封条才成为危险信息；手握瓶身只是拿起，拇指停在瓶盖边缘且迟迟未拧开才形成未完成压力；门外脚步只有逼近并迫使人物第一次移开视线，才成为代价逼近。
- 释放压迫要保留残留，不要复位。若 B 拿起药瓶后听见门外脚步，释放点不是“紧张结束”，而是“拿药动作被打断，药瓶仍在B手中未开，A视线转向门口，下一镜继承门外威胁”。若同镜同时承载拿药和转向门口导致 T2V 不稳，拆成两镜：第一镜只完成药瓶转移，第二镜从药瓶在手的落幅开始处理转向/脚步声。
...
- 焦虑/急迫角色的张力可通过画面收窄、呼吸和手部证据放大；松弛/掌控角色的张力可通过宽松景别、坐姿余裕、固定机位和场景阻力放大。两者同框时，优先让景别差异表达心理距离，而不是让双方都用同等强度的表情或台词抢戏。
...
- `audio_enabled=true` 时，台词、OS、OV 的原文在 `子镜头组` 中必须统一写成半角格式 `{人物}（台词/OS/OV）: "{逐字原文}"`。冒号、空格和英文双引号都必须保留；不得写成中文引号、无引号、无冒号或改写原文。该格式后紧接神态、身体状态、语气、气口和口型边界。
...
- 空间锁定优先使用屏幕坐标和层级坐标：画面左/中/右、前景/中景/背景、人物朝向、视线方向、道具归属、接触点和移动路径。多人关系镜必须明确 180 度轴线，例如“角色A始终在画面左侧面向右，角色B始终在画面右侧面向左，不穿越轴线，不镜像翻转”；道具必须写清“在谁手中/桌面何处/朝向哪里/是否被触碰/落幅归属”。
...
### 4.5.2 当前镜头剧情关键帧与人物站位空间分镜图（自动派生）

Phase 10 自动评估每个主镜的剧情关键帧与人物位置穿帮风险。`当前镜头剧情关键帧` 只为当前主镜导出一张静态生图提示词，锁定本镜最有戏的单一瞬间：观众认知落点、人物关系、关键道具归属、情绪证据、镜头构图和尾帧残留。它不是九宫格，不分 P01-P09，不跨多个主镜，不替代 `full_prompt`，也不得写成首尾帧/I2V/参考素材槽位。

人物位置风险较高时，另行导出两条独立生图提示词：`俯视空间调度图` 锁定场景锚点、人物起终点、路线与摄影机轨迹；`人物站位姿态参考图` 锁定画面左右、前中后景、朝向、视线、重心与道具接触。它们是用户用场景图/人物图生成辅助分镜的生产资料，不是即梦 T2V 正文的一部分，不得写入 `full_prompt` 或伪装为 T2V 参考素材。

剧情关键帧触发优先级：高/关键叙事权重、`rising/peak` 张力、台词刺中听者、关系反转、道具状态改变、压迫/释放机制、重要人物出场、打斗/受力或高抽卡镜。空间分镜图触发优先级：人物/关键道具状态变化为最高风险；服装或道具交接、多人左右关系、可见走位、硬切/重构图/跟拍、多层景深关系依次加权。信息不足时只做合理空间推断，并明确由用户场景图与人物图校正身份、服装和场景细节。
...
- `qa_metadata.ai_model_readiness_score` 是风险触发的 AI 视频大模型可执行性自检。多人、道具/站位变化、`shot_group`、长台词、`rising/peak` 或高抽卡镜必须填写；环境、物件、单人稳定低风险镜可省略。启用时覆盖场景空间、穿帮风险、情绪可读、张力压迫、运镜服务情绪、道具连续、画面美感与 overall，每项 1–10 分并写具体风险，不得空泛好评。
- `qa_metadata.pressure_release_design` 保存压迫感制造与释放合同。仅 `rising/peak` 镜必须填写，Editor 复核它是否落到 `full_prompt` 的压力物、升级点、释放触发和落幅残留；平静镜省略或写 `none`，不得为了提分硬造压力物。
...
- 原生音频开启时，`qa_metadata.dialogue_events[].text` 必须在 `子镜头组` 逐字且只出现一次，并且只能以 `{speaker}（{kind}）: "{text}"` 的格式出现。OS 表示内心独白，OV 表示旁白或画外声，二者都必须使用同一 `: "..."` 包裹格式，同时保持无口型同步。
...
按 `references/format_constraints.md` §B 与验证器结果审查并修复。Editor 只处理语义穿帮和执行竞争；格式错误交给脚本。

审查顺序固定为：

1. 不穿帮硬门槛：人物位置、身体面向、道具归属、口型、尾帧继承、单镜动作预算、状态变化中间态。
2. 剧情节拍：单镜只服务一个 `narrative_beat_id`，子镜只承担起势/承接/转折/落幅，不新增第二目标、第二动作链或第二开放问题。
3. 字段接力：`emotion_driver → camera_beat_map → full_prompt` 不断裂；Camera 不新增剧情/情绪/道具事实，Composer 不重设运镜。
4. 表演与动作：触发、感知来源、反应先后、接触/受力/截停、中断残余、长停顿和打斗因果必须可信。
5. 镜头与空间：景别必须看得见所写表演；运镜、插入镜和注意力交接必须有触发、有承接、不回切抢焦。
6. 声音与平台：台词/OS/OV 逐字保留并按 `{人物}（台词/OS/OV）: "{逐字原文}"` 落地；OS/OV 不驱动口型；禁止 I2V/R2V、参考素材槽位、QA、负面词或工程说明进入 `full_prompt`。
...
- 先满足不穿帮硬门槛，再追求创作表现；稳定失败时拆镜、补定位镜头、减少入画人物或降低运镜，而不是堆叠细节。
- 创作表现权重固定为：剧情节拍与观众认知 > 情绪表演张力 > 景别层次与镜头变化 > 声音氛围和装饰细节。
- 模型只收到可执行的画面、镜头、表演时间轴和平台适用的光声指令。
- 每子镜有唯一注意力中心、有限动作预算和清晰终态。
- 表演由剧情触发，反应次序、物理机制与运动边界可信，主次分明，克制与爆发有反差。
- 每条台词/OS/OV都有确定人物、逐字原文、发声时间窗、可见神态、身体状态、语气和正确口型边界；原生音频开启时必须在子镜头组写成 `{人物}（台词/OS/OV）: "{逐字原文}"`。
- 风险镜必须在 `ai_model_readiness_score` 暴露首轮最弱点；低风险镜只需通过人物位置、道具归属、口型、尾帧继承和动作预算硬门槛。
- 张力按剧情意图校准；高张力来自因果效率、反应延迟与终态残留，平静镜不被强行戏剧化。
- 压迫感按 `pressure_release_design` 执行：先用一个压力物或代价机制收紧观众问题，再用动作完成、打断、注意力转移、代价揭示、延迟悬置或拆镜释放；释放后必须留下可继承状态。
