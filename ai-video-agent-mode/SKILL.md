...
- 主镜头是否只服务一个戏剧问题，子镜是否只承担起势、承接、转折或落幅；是否把第二个剧情目标、第二条主动作链或新的反应结论塞进同一 T2V 任务；
- 子 Agent 字段接力是否完整：`emotion_driver` 是否先于 `camera_beat_map`，`camera_beat_map` 是否消费而非改写情绪重音，Composer 是否只翻译既有镜头设计而未新增运镜或切镜；
- 好奇心钩子是否来自源文已有可见异常、人物反应、暂不揭示原因和马上到来的代价；是否凭空新增悬疑物、设定或多个开放问题；
...
- 景别是否能看见所写表演；
- 景别和运镜是否服务当前镜的戏剧功能；是否为了丰富画面而强行切特写、推拉摇移、插入空镜或使用无触发的景别变化；
- `camera_beat_map` 中每次镜头变化是否有明确 `trigger` 与 `camera_response`；是否出现没有表演重音、道具状态、视线或台词落点支撑的镜头运动；
- 对手戏的景别反差、道具/环境插入是否服务明确心理距离或关系压力；是否出现无信息增量的装饰空镜、过多人物脸部特写、未继承轴线/光源/左右站位的插入镜；
- `continuous_take` 是否出现竞争运镜、焦点或动作；`shot_group` 的反打、切特写、移镜或跟随是否仍服务同一个 `narrative_beat_id`，各自由表情、细部/道具泄露、身体承接或语气落点触发，并在变化后保留人物、道具、轴线和光源承接；
...
- 打斗镜是否按起始距离/站位、攻击意图、接触点或闪避判断、受力方向/重心变化、结果状态和下一镜继承锁连续书写；是否出现同一接触节拍多结果、多段连招、换轴、复杂环绕或夸张慢动作导致穿帮；
- 可见说话者与 supporting 听者同框时，听者是否有且只有一个因果触发的低幅反应、保持闭口且未抢走主体；
- 人物站位、视线、服装、道具、光源和终态是否跨镜承接；
- 相邻主镜是否至少存在动作、视线、道具、声音、空间轴线或情绪残留中的一种承接桥；下一镜起幅是否继承上一镜落幅的姿态、视线、道具接触、重心、屏幕左右、焦点关系与同源光；
...
- `temporal_transition_contract` 是否有真实源文触发与效果依据、唯一效果和有效时间窗；候选未启用时是否明确记录理由；前后状态、声音桥、无口型边界与提示词锚点是否逐项对应；
