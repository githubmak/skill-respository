...
def run():
    ...
    assert field_owner("qa_metadata.scene_tone_palette.tone_palette") == MODEL
    assert field_owner("seedance_prompt") == MODEL
    assert field_owner("seedance_prompt_variants.2.5") == MODEL
    assert field_owner("director_card") == MODEL
