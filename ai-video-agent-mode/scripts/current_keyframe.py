...
def build_keyframe_sequence(task, planned=None, canvas="16:9", visual_style=""):
    ...
    return {
        ...
        "negative_prompt": _keyframe_negative_prompt(metadata),
...
def build_current_shot_keyframe_reference(task, planned=None, canvas="16:9", visual_style=""):
    ...
    pressure = metadata.get("pressure_release_design", {}) if isinstance(metadata.get("pressure_release_design"), dict) else {}
    static_aesthetic = _static_aesthetic_sentence(metadata, 520)
    ...
    prompt_text = (
        ...
        f"美术设计：{static_aesthetic or '以本镜唯一戏眼建立清楚构图、动机光与真实材质'}。"
    ...
    negative = _keyframe_negative_prompt(metadata)
    return {
        "priority": "必需" if _high_priority(metadata, dramatic, pressure) else "建议",
        "reason": _reason(metadata, dramatic, pressure),
...
def _common_keyframe_anchor(task, sections, canvas, visual_style, scene):
    ...
    static_aesthetic = _static_aesthetic_sentence(metadata, 620)
    pieces = [
        "%s画幅" % canvas,
        visual_style,
        "《%s》" % scene,
...
def _frame(label, time_seconds, common, state, prop_state, purpose):
    prompt = (
        "%s。%s；状态：%s；道具：%s；%s。"
        "单张静态画面，不分格；人物年龄性别、发型服装、位置朝向与道具状态保持明确，"
        "身体和道具真实接触环境，光源方向、受光面、焦点层级与空间纵深清楚。"
...
def _keyframe_video_prompt(task, frames, t2v_prompt):
    ...
    metadata = task.get("qa_metadata", {}) if isinstance(task.get("qa_metadata"), dict) else {}
    dynamic_aesthetic = _dynamic_aesthetic_sentence(metadata, 680)
    return (
        "%s0.0-%.1f秒：从起始状态稳定进入戏眼，不改变人物身份、空间、支撑点和道具归属；"
        "%.1f-%.1f秒：只执行已确认主动作与表演变化，保持可见面、视线和接触路径连续；"
        "%.1f-%.1f秒：动作收束到结束关键帧并稳定保持。即梦执行正文：%s"
        % (
            ("动态美术：%s。" % dynamic_aesthetic) if dynamic_aesthetic else "",
            middle, middle, max(middle, end * 0.85), max(middle, end * 0.85), end, direct,
        )
    )


def _static_aesthetic_sentence(metadata, limit=520):
    ...
    return _compact("；".join(str(contract.get(field, "") or "").strip() for field in fields if str(contract.get(field, "") or "").strip()), limit)


def _dynamic_aesthetic_sentence(metadata, limit=680):
...
def _keyframe_negative_prompt(metadata=None):
    metadata = metadata if isinstance(metadata, dict) else {}
    contract = metadata.get("static_aesthetic_contract", {}) if isinstance(metadata.get("static_aesthetic_contract"), dict) else {}
    scene_exclusions = _compact(contract.get("aesthetic_exclusions", ""), 240)
    base = (
        "九宫格, 多格漫画, P01, P09, 面板标签, 分镜编号, 连环画排版, 新增人物, "
        "新增道具, 人物换位, 左右颠倒, 支撑点漂移, 接触穿插, 道具瞬移, 功能面翻转, "
        "人体比例突变, 近远比例失调, 光源跳变, 肤色污染, 均匀棚拍光, 平均构图, "
        "塑料皮肤, 廉价CG质感, 过度霓虹, 文本气泡, 水印"
    )
    return base + ((", " + scene_exclusions) if scene_exclusions else "")
