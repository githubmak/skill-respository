#!/usr/bin/env python3
"""Check saved storyboard structure and timing; never judge cinematic quality."""
import argparse
import re
import sys
from pathlib import Path

SHOT = re.compile(r"^### 镜头\s+([^｜|]+?)\s*[｜|]\s*时长[：:]\s*(\d+(?:\.\d+)?)\s*(?:秒|[sS])", re.M)
...
def validate(raw, mode):
    ...
    for i, scene in enumerate(scenes):
        ...
        for key in ["场景资产图提示词", "道具资产图提示词", "场景空间位置关系", "Seedance 完整独立镜头"]:
    ...
    if not matches:
        errors.append("未找到镜头标题")
        return errors
    ids = set()
    total = 0.0
    for i, match in enumerate(matches):
        sid, duration = match.group(1).strip(), float(match.group(2))
        if sid in ids:
            errors.append(sid + "：镜号重复")
        ids.add(sid)
        total += duration
        if duration <= 0 or duration > 35:
            errors.append(sid + "：单镜时长必须大于0且不超过35秒")
        ...
        for j, window in enumerate(windows):
            ...
            parts = re.split(r"[｜|]", window.group(3))
            if abs(start - previous) > 0.05 or end <= start:
                errors.append(sid + "：时间段不连续或起止无效")
            previous = end
            if len(parts) not in (2, 3) or not re.search(r"\d+(?:\.\d+)?\s*mm", parts[0]) or not SIZES.search(" ".join(parts[:-1])) or not parts[-1].strip():
            ...
            if not any(line.startswith("声音：") for line in lines):
                errors.append(sid + "：时间段缺少独立的声音行")
            if not any(not line.startswith("声音：") for line in lines):
    ...
    elif abs(float(declared.group(1)) - total) > 0.5:
